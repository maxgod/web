...
class LexicalService
{
    ...
    public $word = array(T_IMG=>'img',T_PAGE=>'page',T_IF=>'if',T_ELSE=>'else',T_ELSEIF=>'elseif',T_FOR=>'for',T_FOREACH=>'foreach',T_SWITCH=>'switch',T_CASE=>'case',T_DEFAULT=>'default',T_ECHO=>'echo',T_ENDIF=>'endif',T_ENDFOR=>'endfor',T_ENDFOREACH=>'endforeach',T_ENDSWITCH=>'endswitch',T_BREAK=>'break');
     ...
     * @return string $html
     */
    public function template($html = "",$is_cache = false)
    {
        ...
        foreach($result[1] as $key=>$val)
        {
            ...
            {
               //词法处理
                $Replace[] = $this->keyword_handle($lexical);
            }
            else
            {
                //非词法处理
    ...
    public function keyword_handle($lexical)
    {
        ...
        if($count)
        {
            switch($key)
            {
                ...
                case T_PAGE:
                    ...
                    return PAGE(trim($lexical[1],"'"),$param);
                    break;
                ...
                case T_IMG:
                    ...
                    if(isset($lexical[2]))
                    {
                        $args = explode(",",trim($lexical[2],"'"));
                        foreach($args as $val)
                        {
                            $str = explode('=',$val);
                            if($str[0] == "width" )
                                $width = rtrim($str[1],'px');
                            if($str[0] == "height" )
                                $height = rtrim($str[1],'px');
                            $param .= $str[0]."='".$str[1]."' ";
                        }
                    }
                    return images(trim($lexical[1],"'"),$param,$width,$height);
                    break;
                default:
        ...
        {
            $symbol = $this->symbol[T_SR];
            if($key == T_ELSE)
                $symbol = $this->symbol[T_COLON];
            $string = " {$this->word[$key]}{$symbol} ";
        }

        return $this->symbol[T_OPEN_TAG].$string.$this->symbol[T_CLOSE_TAG];
