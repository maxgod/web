...
function getQqLoginUrl($appid,$mobile = 0,$site_path)
{
	$state = md5(uniqid(rand(),TRUE));
    $url = "https://graph.qq.com/oauth2.0/authorize?response_type=code&client_id="
        . $appid . "&redirect_uri=".urlencode($site_path."callback/qq.php")
        . "&state=" .$state
        . "&scope=".QQ_SCOPE;
...
function getQqAccessToken($appid,$appkey,$site_path)
{
	$qq_state = $_SESSION['qq_state'];
	if($_REQUEST['state'] == $qq_state)
    {
        $token_url = "https://graph.qq.com/oauth2.0/token?grant_type=authorization_code&"
            . "client_id=" . $appid. "&redirect_uri=" . urlencode($site_path."callback/qq.php")
