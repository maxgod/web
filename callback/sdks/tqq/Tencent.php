...
class TqqOAuth
{
    ...
    {
        $r = json_decode(TencentTqq::api('user/info'), true);
        if ($r['data']['name']) {
            return true;
        } else {
            self::clearOAuthInfo();
    ...
    {
		global $_Global;
		if(isset($_Global['login_oauth']['tqq']))
			unset($_Global['login_oauth']['tqq']);
    }
}
...
class TencentTqq
{
    ...
    {
		global $_Global;
        if (isset($_Global['login_oauth']['tqq']['t_access_token'])) {//OAuth 2.0 方式
            //鉴权参数
            $params['access_token'] = $_Global['login_oauth']['tqq']['t_access_token'];
            $params['oauth_consumer_key'] = TqqOAuth::$client_id;
            $params['openid'] = $_Global['login_oauth']['tqq']['t_openid'];
            $params['oauth_version'] = '2.a';
            $params['clientip'] = TqqCommon::getClientIp();
            $params['scope'] = 'all';
        ...
        } elseif (isset($_Global['login_oauth']['tqq']['t_openid']) && isset($_Global['login_oauth']['tqq']['t_openkey'])) {//openid & openkey方式
            $params['appid'] = TqqOAuth::$client_id;
            $params['openid'] = $_Global['login_oauth']['tqq']['t_openid'];
            $params['openkey'] = $_Global['login_oauth']['tqq']['t_openkey'];
            $params['clientip'] = TqqCommon::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
            ...
            //生成签名
            $urls = @parse_url($url);
            $sig = TqqSnsSign::makeSig($method, $urls['path'], $params, TqqOAuth::$client_secret.'&');
            $params['sig'] = $sig;
        }
        
        //请求接口
        $r = TqqHttp::request($url, $params, $method, $multi);
